...
{
    // soket oluştur. (udp)
    // dns isteği header + query + question dan oluşur.  bunları oluştur paket gönder
    // header oluşturma 
    // dotted2DNS ile host ismini dns formatına çevir
    // query oluştur  
    // recieve from ile cevabı al.
    // header ı oku printresponse header info 
    // read answers, read authorities, read additional sonra print
    ...
    struct RES_RECORD answers[RSP_SIZE], auth[RSP_SIZE], addit[RSP_SIZE];
    unsigned char *reader;
    int stop, recv_size, i;
    socklen_t len;
    ...
    dest.sin_port = htons(DNS_PORT); // big-endian port
    dest.sin_addr.s_addr = inet_addr(dns_servers[dst_dns_server]);

    for( i=0;i<BUFF_SIZE;i++) buf[i]=0; // temizleme

    dns = (struct DNS_HEADER *)&buf;// buffer ın başı header

    fillQueryHeader(dns); // header doldur

    // qname 
    qname = (unsigned char*)&buf[sizeof(struct DNS_HEADER)]; // header dan sonraki kısım
    dotted2DNS(qname , host);

    // question 
    qinfo = (struct QUESTION*)&buf[ sizeof(struct DNS_HEADER) + (strlen((const char*)qname) + 1) ]; // header + qname dan sonraki
    ...
    dns = (struct DNS_HEADER*)buf; // bufferın başı header

    // response header oku
    printResponseHeaderInfo(dns);

    // cevapları oku
    reader = &buf[ sizeof(struct DNS_HEADER) + (strlen((const char*)qname)+1) + sizeof(struct QUESTION) ]; // header + qname + question dan sonraki
    stop = 0;
...
void fillQueryHeader(struct DNS_HEADER *reqHeader) {
    srand(time(NULL));
    reqHeader->id = htons(rand() % BUFF_SIZE);

    reqHeader->qr = 0;      // query
    reqHeader->opcode = 0;  // standard
    reqHeader->aa = 0;      
    reqHeader->tc = 0;      
    reqHeader->rd = 1;      // recursive
